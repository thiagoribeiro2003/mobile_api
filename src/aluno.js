...
function lerUmEvento(id, res) {
  const sql =
    "SELECT * FROM evento LEFT JOIN perfil ON evento.perfil_id = perfil.id WHERE perfil_id = ?";
...
function atualizarEvento(id, evento, res) {
...
function excluir(id, res) {
  ...
  conexao.query(sql, id, (erro, resultados) => {
    if (erro) {
      ...
      res.status(200).json({ status: "perfil excluído", id });
    }
  });
}

function excluirEvento(id, res) {
  ...
  conexao.query(sql, id, (erro, resultados) => {
    if (erro) {
      ...
      res.status(200).json({ status: "evento excluído", id });
    }
  });
}
